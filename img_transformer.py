...
class ImgTransformer(object):
    ...
    def image_to_block(self, image_sets, b_size=3, pad_en=True):
        ...
        :param image_sets:  a numpy array of RGB image sets with shape (image_cnt_per_set, height, width, 3)
        :param b_size:  value for the height and width of one block
        :param pad_en:  switch for padding operation
        :return:  a numpy array of blocks with shape (n_block, b_size, b_size, 3)
        """
        blocks = []
    ...
    def image_to_patch(self, image_sets, p_size=3, pad_en=True):
        ...
        :param image:  a numpy array of RGB image sets with shape (image_cnt_per_set, height, width, 3)
        :param p_size:  value for the height and width of one patch
        :param pad_en:  switch for padding operation
        :return:  a numpy array of patches with shape (n_patch, p_size, p_size, 3)
        """
        patches = []
        for image in image_sets:
            if not pad_en:
