...
## Convolution Layers
As in Figure 6 (Niklaus 2017 CVPR), we use an identical model as Niklaus et al. use in their paper, we use two receptive blocks with size 79 x 79 x 3 combined to 79 x 79 x 6 as input. 
...
Niklaus et al. have proposed a new method in their later research to optimize the memory consumption. As shown in Figure 10 (Niklaus 2017 ICCV), first, instead of generating a 2-D kernel for each pixel, their new network produces four 1-D kernels for each pixel.   
